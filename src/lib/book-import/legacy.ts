// v1 (legacy) book extractor — preserved verbatim from the original
// cognitive-backend/src/lib/book-import.ts so that BOOK_IMPORT_PARSER=v1 keeps
// the previous behavior available during the transition. Do NOT add features
// here; new work belongs in the v2 pipeline (epub/, pdf/).
...
async function extractFromEpub(buffer: Buffer): Promise<ExtractedBookContent> {
  ...
  const readerPayload: ExtractedEpubReaderPayload = {
    format: 'epub',
    version: 1,
    documents,
  };
...
export async function extractBookContentFromUploadV1(params: {
  buffer: Buffer;
  originalName: string;
  mimeType: string;
